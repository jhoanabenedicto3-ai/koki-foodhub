(function(){
  ...
  function renderChart(data){
    ...
    // Ensure forecast exists and has meaningful (positive) values; otherwise generate a client-side fallback
    const forecastRaw = (data.forecast || []).slice();
    const forecastValidCount = forecastRaw.filter(v => v !== null && v !== undefined && Number(v) > 0).length;
    let finalForecast = forecastRaw.slice();
    if (!finalForecast || finalForecast.length === 0 || forecastValidCount === 0) {
    ...
    salesChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: { legend: { display: true, position:'top', labels: { usePointStyle:true } }, tooltip: { mode:'index', intersect:false, callbacks:{ label: function(ctx){ if (ctx.raw === null) return ctx.dataset.label + ': —'; const value = (window.currencySymbol||'₱') + Number(ctx.raw).toLocaleString('en-PH'); if (/forecast|project/i.test(ctx.dataset.label)) { return 'Projected: ' + value; } return ctx.dataset.label + ': ' + value; } } } },
